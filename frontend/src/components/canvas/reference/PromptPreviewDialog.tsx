    ...
    >
      <div className="flex max-h-[86vh] flex-col">
        <div className="flex items-start gap-3 border-b border-[var(--color-hairline-soft)] px-5 py-4">
          <span
            aria-hidden
            className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-[var(--color-accent)]/25 bg-[var(--color-accent-soft)] text-[var(--color-accent-2)]"
          >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-[11.5px] leading-relaxed text-[var(--color-text-4)]">
              只编译，不生成视频、不调用供应商、不产生生成费用。内容按当前未保存正文草稿与当前高级参数计算。
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <Meta
                  label="编译器"
                  value={`${preview.prompt_compiler}${preview.compiler_applied ? " · applied" : ""}`}
                />
                ...
                <Meta
                  ...
                  value={
                    maxChars
                      ? `${preview.prompt_chars.toLocaleString()} / ${maxChars.toLocaleString()}`
                      : preview.prompt_chars.toLocaleString()
                  }
                  warn={nearLimit}
                />
              </div>
                  ...
                  <div className="grid gap-1.5">
                    {preview.reference_mapping.map((item) => (
                      <div
                        ...
                        className="grid grid-cols-[72px_88px_1fr] items-center gap-2 rounded-md border border-[var(--color-hairline-soft)] px-2.5 py-1.5 text-[11px]"
                      >
                        <code className="text-[var(--color-text-4)]">{item.picture}</code>
                        <code className="text-[var(--color-accent-2)]">{item.subject}</code>
                        <span className="truncate text-[var(--color-text-2)]" title={item.label}>
                          {item.label}
                        </span>
                      </div>
                    ))}
              ...
              <div className="overflow-hidden rounded-lg border border-[var(--color-hairline)] bg-[oklch(0.13_0.006_265)]">
                <div className="flex items-center justify-between border-b border-[var(--color-hairline-soft)] px-3 py-2">
                  <div className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-4)]">
                ...
                <pre className="max-h-[48vh] overflow-auto whitespace-pre-wrap break-words p-3 font-mono text-[11px] leading-[1.65] text-[var(--color-text-2)]">
                  {prompt}
                </pre>
              </div>
              ...
              {preview.rendered_prompt !== preview.provider_prompt && (
                <details className="rounded-lg border border-[var(--color-hairline-soft)] bg-[oklch(0.17_0.008_265_/_0.45)]">
                  <summary className="cursor-pointer px-3 py-2 text-[11px] font-medium text-[var(--color-text-3)]">
                    查看 H3 编译前的 ArcReel provider prompt
                  </summary>
                  <pre className="max-h-[28vh] overflow-auto whitespace-pre-wrap break-words border-t border-[var(--color-hairline-soft)] p-3 font-mono text-[10.5px] leading-relaxed text-[var(--color-text-3)]">
...
function Meta({
  ...
  return (
    <div className="rounded-lg border border-[var(--color-hairline-soft)] bg-[oklch(0.18_0.010_265_/_0.5)] px-3 py-2">
      <div className="text-[9.5px] font-bold uppercase tracking-wider text-[var(--color-text-4)]">
        {label}
      </div>
      <div className={`mt-1 truncate font-mono text-[11px] ${warn ? "text-amber-300" : "text-[var(--color-text-2)]"}`}>
        {value}
      </div>
    </div>
  );
