...
<Subject 2> is {YOUNG_CHARACTER_NAME}, the younger-age appearance reference for canonical character {CHARACTER_ID} {CHARACTER_NAME}, derived from <Picture 2>. Preserve the younger age, hairstyle, clothing, and child proportions from <Picture 2>, while locking the core facial identity to the shared canonical C03 identity master in <Picture 4>.
<Subject 3> is canonical character {CHARACTER_ID} {CHARACTER_NAME}, the later-age appearance reference derived from <Picture 3>. Preserve the later-age hairstyle, clothing, and proportions from <Picture 3>, while locking the core facial identity to the SAME shared canonical C03 identity master in <Picture 4>.
<Picture 4> is the shared canonical C03 {CHARACTER_NAME} identity master for BOTH <Subject 2> and <Subject 3>. It supplies the common facial identity only: eye geometry and spacing, brows, nose, mouth, cheek structure, hairline family, skin tone, and recognizable likeness. It is not a fourth person.
<Subject 2> and <Subject 3> are the SAME PERSON at two ages. Never swap, blend, replace, recast, or invent either face.

summary:
[reference generation] Create one continuous 15-second horizontal cinematic memory sequence in three authored 5-second shots. CHARACTER IDENTITY LOCK — HIGHEST PRIORITY: [Shot 2] must use <Subject 2> from <Picture 2> with <Picture 4> as the shared C03 identity master; [Shot 3] must use <Subject 3> from <Picture 3> with the SAME <Picture 4> identity master. The two are the same person at different ages. Never generate a generic child, never substitute a different girl, never reinterpret <Picture 4> as a separate character, and never allow identity drift. VISIBLE-TEXT LOCK — HIGHEST PRIORITY: across every frame, the ONLY readable or legible language is the exact Chinese phrase “{ONLY_VISIBLE_TEXT}”, allowed only on the smartphone alarm label in [Shot 1]. Do not create any other readable letters, Chinese characters, words, numbers, clock digits, status-bar text, labels, buttons, titles, logos, watermarks, captions, subtitles, karaoke text, speech bubbles, or glyph-like pseudo-text. DIALOGUE CHANNEL LOCK — HIGHEST PRIORITY: Dialogue enclosed by <d> is AUDIO-ONLY. Never typeset, transcribe, caption, subtitle, echo, or otherwise visualize any content from <d>.
...
<Subject 2> (appears in [Shot 2]): fully_preserved age appearance + shared identity - preserve age-specific appearance from <Picture 2>, but use <Picture 4> as the authoritative common facial identity. Keep the shared eyes, brows, nose, mouth, cheeks, hairline family, skin tone, and recognizable C03 likeness. Do not replace it with another child.
<Subject 3> (appears in [Shot 3]): fully_preserved age appearance + shared identity - preserve age-specific appearance from <Picture 3>, but use the SAME <Picture 4> as the authoritative common facial identity. Keep the shared eyes, brows, nose, mouth, cheek/jaw relationship, hairline family, skin tone, and recognizable C03 likeness. Do not replace it with another child.
Shared identity master: <Picture 4> is the authoritative common facial identity for both ages of C03. It must bind both memory shots to one person rather than act as a separate visible subject.
Cross-age continuity: <Subject 2> and <Subject 3> must read as the SAME PERSON at two ages. No face substitution, no random child casting, no identity swap, no identity morph into a third face.

detailed_description:
CHARACTER IDENTITY CONTRACT — HIGHEST PRIORITY: the child in both memory shots is canonical {CHARACTER_ID} {CHARACTER_NAME}. <Picture 4> is the shared facial-identity master for BOTH memory shots. [Shot 2] takes younger age styling and proportions from <Picture 2> but facial identity from <Picture 4>. [Shot 3] takes later-age styling and proportions from <Picture 3> and the SAME facial identity from <Picture 4>. The viewer must recognize these as two ages of one person. Do not synthesize either child from prose alone, do not merge the references into a third face, and do not let eye spacing, brows, nose, mouth, cheeks, hairline family, skin tone, or overall likeness drift from the shared identity master.
...
[Shot 2] 00:05-00:10. Clean hard cut to a warm, softened phone-memory flashback. <Subject 2> is the only child face permitted in this shot. Keep the younger age, hair, clothing, and proportions from <Picture 2>, while matching the shared C03 facial identity in <Picture 4>. She holds a phone with its screen turned away from camera or fully defocused so no UI can be read. She looks upset and calls for her mother. No captions or subtitles. <Subject 2> says in an emotional natural child voice, <d>[Chinese] {DIALOGUE_1}</d>. Slight 50mm push-in, shallow depth of field, subtle telephone-room tone. The spoken words remain audio-only.

[Shot 3] 00:10-00:15. Clean hard cut to a later memory of THE SAME PERSON. <Subject 3> is the only child face permitted in this shot. Keep the later-age appearance from <Picture 3>, while matching the SAME shared C03 facial identity in <Picture 4>. Do not reuse a generic face from Shot 2 and do not invent a third face. She glances away with mild impatience while holding the phone low; any screen surface is turned away, dark, or heavily defocused. No readable text anywhere in frame. <Subject 3> says in a casual distracted child voice, <d>[Chinese] {DIALOGUE_2}</d>. Static 50mm medium close-up. End on the child's averted gaze. The spoken words remain audio-only.
