"""Formal delivery QA for the exact v3.3 ArcReel import archive.

The archive under test is reconstructed byte-for-byte by the workflow and its SHA-256
is checked before this test starts.  This test intentionally uses ArcReel's native
ProjectArchiveService import path, then compiles H3 Ref2VA previews from the imported
scripts for both a 10-second/two-shot unit and a 15-second/three-shot unit.
"""
...
def _asset(name: str, asset_type: str) -> dict:
...
def _preview(unit: dict, assets: list[dict]) -> dict:
    ...
    assert all(section in preview["provider_prompt"] for section in REQUIRED_SECTIONS)
    return preview


def test_formal_v33_exact_archive_native_import_and_h3_preview(tmp_path: Path) -> None:
    package_env = os.environ.get("FORMAL_PACKAGE_ZIP")
    assert package_env, "FORMAL_PACKAGE_ZIP is required"
    ...
    for episode_num in range(1, 16):
        ...
        assert len(units) == 6
        assert [unit["duration_seconds"] for unit in units] == EXPECTED_DURATIONS
    ...
    assert all((imported_root / "source" / f"episode_{n}.md").is_file() for n in range(1, 16))

    e1 = pm.load_script(project_name, "scripts/episode_1.json")
    e13 = pm.load_script(project_name, "scripts/episode_13.json")
    ten_second = e1["video_units"][0]      # E1U01: 10 sec / 2 shots
    fifteen_second = e13["video_units"][2]  # E13U03: 15 sec / 3 shots
    ...
    assert "<d>[Chinese] 打开底层日志</d>" in preview_15["provider_prompt"]

    # Determinism: the exact same imported unit + transmitted reference order must
    # produce the exact same provider prompt SHA on a second compile.
    preview_10_repeat = _preview(
        ten_second,
        [
            _asset("M国机场", "scene"),
            _asset("沈知意", "character"),
    ...
    # Native export after import must also succeed. Re-import the native export into
    # a clean ProjectManager to prove round-trip archive compatibility.
    exported_zip, _ = service.export_project(project_name)
    try:
        roundtrip_pm = ProjectManager(tmp_path / "roundtrip-projects")
        ...
        roundtrip_e13 = roundtrip_pm.load_script(roundtrip.project_name, "scripts/episode_13.json")
        assert [u["duration_seconds"] for u in roundtrip_e13["video_units"]] == EXPECTED_DURATIONS
